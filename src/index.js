...
(async function () {
    try {
        ...
        if(args.mode === 0) {
            ...
            newNames.map(({oldFile, newFile}) => 
                fs.link(oldFile, basePath + newFile, err => err ? console.log(err): ""));
        } else if(args.mode === 1) {
            console.log("Creating Symlinks");
            newNames.map(({oldFile, newFile}) => 
                fs.symlink(oldFile, basePath + newFile, err => err ? console.log(err): ""));
        } else if(args.mode === 2) {
            console.log("Renaming Files");
            newNames.map(({oldFile, newFile}) => 
                fs.rename(oldFile, basePath + newFile, err => err ? console.log(err): ""));
            //It will deal with all the srt, false positives in movies, and tv shows and other files
            await Promise.all(other.map(async file => await whatToDoWithFile(file, basePath))); 
...
function findNewNamesForFiles({shows, showsData, movies, moviesData}) {
    let names = [];
    Object.keys(movies)
        .map(name => names.push(findNewNameForMovie({name, "file": movies[name], moviesData})));
    Object.keys(shows)
        .map(name => names = [...names, ...findNewNameForShow({name, files: shows[name].files, showsData})]);
...
function filterShowsAndMovies(video) {
    ...
    video.map(({file, type, fileStats, name}) => {
        ...
        }
        {
            ...
            //New show detected
            if(!sameShow) { shows[name] = {season: [season], length: 1, files: [{file, episodeNum, season}]}; return; } 
            if(shows[sameShow.name] && shows[sameShow.name].hasOwnProperty("files")) {
                shows[sameShow.name].files.push({file, episodeNum, season});
            }
